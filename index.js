...
/******** OWNER NUMBER**********/
const ownerNumber = ["556996103915@s.whatsapp.net","556996103915@s.whatsapp.net"] 
/************************************/
          ...
          const checkLimit = (sender) => {
           ...
           return false
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    try { 
                        reply("TOME CUIDADO COM A FALA DE AMIGO!! 😠")
                        setTimeout( () => {
 	                           client.groupLeave(from) 
 					   	}, 5000)
								setTimeout( () => {
                    ...
                    return reply( "Por favor, tome cuidado 😇")
                }
            }
        }
			...
			switch(command) { 
				...
				case 'tahta':
					...
					har = body.slice(12)
					reply('「❗」Hirti Freqüentemente Tai Anjg :v')
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                ...
                if (umurUser < 12) return reply(`sua idade é muito jovem, no mínimo 12 anos »)
                try {
...
• Limit: ${limitawal-updated.limit}`
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				const dare =['Envie uma mensagem para o seu ex e diga "Eu ainda gosto de você ',' ligue para a namorada / namorada agora e ss para o jogador ',' papai ke salah satu anggota grup','Bilang "KAMU CANTIK BANGET NGGAK BOHONG" ke cowo','ss recent call whatsapp','drop emot "🦄??" setiap ngetik di gc/pc selama 1 hari','kirim voice note bilang can i call u baby?','drop kutipan lagu/quote, terus tag member yang cocok buat kutipan itu','pake foto sule sampe 3 hari','ketik pake bahasa daerah 24 jam','ganti nama menjadi "gue anak lucinta luna" selama 5 jam','chat ke kontak wa urutan sesuai %batre kamu, terus bilang ke dia "i lucky to hv you','prank chat mantan dan bilang " i love u, pgn balikan','record voice baca surah al-kautsar','bilang "i hv crush on you, mau jadi pacarku gak?" ke lawan jenis yang terakhir bgt kamu chat (serah di wa/tele), tunggu dia bales, kalo udah ss drop ke sini','sebutkan tipe pacar mu!','snap/post foto pacar/crush','teriak gajelas lalu kirim pake vn kesini','pap mukamu lalu kirim ke salah satu temanmu','kirim fotomu dengan caption, aku anak pungut','teriak pake kata kasar sambil vn trus kirim kesini','teriak " anjimm gabutt anjimmm " di depan rumah mu','ganti nama jadi " BOWO " selama 24 jam','Pura pura kerasukan, contoh : kerasukan maung, kerasukan belalang, kerasukan kulkas, dll']
